...
rows = [
    ...
    ("07_月次集計", "上司報告・請求突合用。★稼働率・区分別・利用者別が出ます。"
                 "期間を空欄にしておけば対象月に連動し、日付を入れれば年度など任意の期間で集計できます。"),
    ...
    ("1つのファイルを使い続けます", "月ごとに新しいファイルを作る必要も、シートを増やす必要もありません。"
                       "予約は 01_予約入力 に下へ追記していくだけです。"),
    ("★変えるのは1か所だけ", "02_カレンダー の「対象月」を変えると、次のシートがまとめて切り替わります。"
                    "　02_カレンダー ／ 03_帯表 ／ 04_月間表 ／ 06_食数表 ／ 09_FAX空き表 ／ "
                    "07_月次集計 ／ 08_支給量管理。"),
    ("07と08は上書きもできます", "07_月次集計 の開始日・終了日、08_支給量管理 の対象月は、空欄にしておけば"
                       "対象月に連動します。年度でまとめたいときなど、別の期間を見たいときだけ"
                       "日付を直接入れてください（入れた月が優先されます）。"),
    ("連動しないシート", "05_空室照会 だけは連動しません。「この期間空いていますか」と聞かれたその場で"
                  "日付を2つ入れて使うシートなので、対象月とは関係なく動きます。"),
...
label_input(ws, 3, "開始日", None, "空欄にすると 02_カレンダー の対象月の1日になります", "yyyy/mm/dd")
label_input(ws, 4, "終了日", None, "空欄にすると 02_カレンダー の対象月の末日になります", "yyyy/mm/dd")
# 実際に使う期間（空欄なら対象月）。J列は作業用として隠す。
ws["J3"] = f'=IF($B$3="",{M1},$B$3)'
ws["J4"] = f'=IF($B$4="",EOMONTH({M1},0),$B$4)'
for cc in ("J3", "J4"):
    ws[cc].number_format = "yyyy/mm/dd"; ws[cc].font = C_AUTO
ws.column_dimensions["J"].hidden = True
S_, E_ = "$J$3", "$J$4"
label_auto (ws, 5, "集計する期間",
            f'=YEAR({S_})&"年"&MONTH({S_})&"月"&DAY({S_})&"日　〜　"'
            f'&YEAR({E_})&"年"&MONTH({E_})&"月"&DAY({E_})&"日"',
            "年度でまとめたいときは 2026/4/1 と 2027/3/31 のように直接入れてください")
ws["B5"].font = Font(name=FONT, size=11, bold=True, color="1F3B52")
ws["B6"] = f'=IF({E_}<{S_},"⚠ 終了日が開始日より前です。日付を確認してください","")'
...
SLOT = f'(($J$4-$J$3+1)*{CAP})'
ws["A9"] = '=IF($J$4<$J$3,"",$J$4-$J$3+1&" 日")'
ws["B9"] = f'={CAP}'
ws["C9"] = f'=IF($J$4<$J$3,"",{SLOT})'
ws["D9"] = f'=IF($J$4<$J$3,"",SUMPRODUCT({VALID}*{CONF}*{NIGHTS}))'
ws["E9"] = f'=IF(OR($J$4<$J$3,{SLOT}=0),"",$D$9/{SLOT})'
ws["F9"] = f'=IF($J$4<$J$3,"",SUMPRODUCT({VALID}*{TENTV}*{NIGHTS}))'
ws["G9"] = f'=IF(OR($J$4<$J$3,{SLOT}=0),"",($D$9+$F$9)/{SLOT})'
...
label_input(ws, 3, "対象月", None, "空欄にすると 02_カレンダー の対象月に連動します", "yyyy/mm/dd")
ws["J3"] = f'=IF($B$3="",{M1},DATE(YEAR($B$3),MONTH($B$3),1))'
ws["J3"].number_format = "yyyy/mm/dd"; ws["J3"].font = C_AUTO
ws.column_dimensions["J"].hidden = True
label_auto(ws, 4, "集計する月", '=YEAR($J$3)&"年"&MONTH($J$3)&"月"',
           "この月に入所した予約を数えます")
ws["B4"].font = Font(name=FONT, size=11, bold=True, color="1F3B52")
